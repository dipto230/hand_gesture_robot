...
void controlFinger(int index, int state) {
  ...
  if (index == 0 || index == 1) {
    ...
    moveServoSmooth(index, targetAngle, 30); // 30ms steps for smooth movement
  } else if (index == 3) {
    // Pin 4 (clockwise): 1=open(180°), 0=fold(0°) - SLOWER to reduce noise
    targetAngle = state == 1 ? 180 : 0;
    moveServoSmooth(index, targetAngle, 50); // 50ms steps = slower, less noise
  } else {
    // Pin 5, 3 (normal): 1=open(180°), 0=fold(0°)
    targetAngle = state == 1 ? 180 : 0;
    moveServoSmooth(index, targetAngle, 30);
  }
}
